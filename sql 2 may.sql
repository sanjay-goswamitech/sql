...
select count(amount) , sales_person,sum(sale_date) from sales where sales_person='amit' group by month(sale_date)=01;
-- 2 may

drop table if exists sales;

select count(sales_person) from sales;
-- how many sales are done by each person 
select sales_person,count(sales_person) from sales group by sales_person;
-- sum(amount) done by each person in the dicrising order 
select count(sales_person),sales_person,sum(amount)  from sales group by sales_person order by count(amount) desc;
